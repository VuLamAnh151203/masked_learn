...
class FULL_DETERMINISTIC_WEAK_IB_MASK(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        # -------------------------------------------------------------
        # Deterministic weak information-bottleneck regularization.
        #
        # IMPORTANT:
        #   ib_beta = 0.0 makes training use the original Full + Mask
        #   objective exactly. No new learnable modules/parameters are
        #   introduced and the original mask initialization is unchanged.
        # -------------------------------------------------------------
        self.ib_beta = float(
            config['ib_beta']
        ...
        self.ib_prior_retention = float(
            config['ib_prior_retention']
            if config['ib_prior_retention'] is not None
            else 0.70
        )
        ...
        if self.ib_beta < 0.0:
            raise ValueError("ib_beta must be >= 0.")
    ...
    def calculate_loss(self, interaction):
        ...
        # -------------------------------------------------------------
        # ORIGINAL Full + Mask loss, kept exactly in log2 form.
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # Clean ablation:
        # ib_beta == 0.0 -> exactly the original optimization objective.
        # -------------------------------------------------------------
        if self.ib_beta == 0.0:
            return loss_value

        # -------------------------------------------------------------
        # Deterministic soft-mask information bottleneck.
        #
        # The graph mask itself is still:
        #
        #     M_e = sigmoid(mask_logits[e])
        #
        # No stochastic sampling and no hard edge dropping.
        # -------------------------------------------------------------
        mask = torch.sigmoid(
            self.mask_logits
        ).clamp(

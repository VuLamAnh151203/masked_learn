...
GLORIA with TWO GCN views using:
    ...
    1) a learnable per-edge SHARED backbone S
    2) an EDGE-CONDITIONED competitive private router

The private router is NOT one free parameter pair per edge anymore.
Instead, a shared MLP receives structural features of each interaction edge.

For every user-item edge e=(u,i), define:
...
where d_u and d_i are normalized log-degrees in [0,1].

Degree prior
------------
The initial private preference is:

    niche_score      = d_u * (1 - d_i)
    popularity_score = (1 - d_u) * d_i

and the base router logits are:

    prior_logits_e =
        temperature * prior_scale
        * [niche_score, popularity_score]

Learned residual correction
---------------------------
A small shared MLP learns:

    delta_logits_e = MLP(x_e)

The final router logits are:

    router_logits_e =
        prior_logits_e + delta_logits_e

and competitive routing is:

    [q1_e, q2_e] =
        softmax(router_logits_e / temperature)

so:

    q1_e + q2_e = 1.

The final graph masks are:
...
--------------
S:
    shared collaborative backbone visible to BOTH GCNs.

q1:
    niche-oriented private routing.

q2:
    popularity-oriented private routing.

The degree formula is only a PRIOR. The MLP learns a global edge-routing rule
that can correct that prior according to endpoint structural properties.

The last MLP layer is initialized to zero, so at epoch 0:

    delta_logits = 0

and routing starts exactly from the degree prior.
...
No extra diversity loss is used by default.
"""
...
class EDGE_CONDITIONED_COMPETITIVE_MASKS(GeneralRecommender):
    def __init__(self, config, dataset):
        super(EDGE_CONDITIONED_COMPETITIVE_MASKS, self).__init__(config, dataset)
        ...
        # -------------------------------------------------------------
        # Shared-backbone + edge-conditioned competitive router settings.
        # -------------------------------------------------------------
        ...
        # Strength of the hand-designed degree prior.
        # 0.0 means the router initially has no niche/popularity preference.
        self.router_prior_scale = float(
            _cfg(config, 'router_prior_scale', 2.0)
        )

        # Hidden width of the shared edge-conditioned residual MLP.
        self.router_hidden_dim = int(
            _cfg(config, 'router_hidden_dim', 32)
        )

        # Optional multiplier on the learned MLP correction.
        # Keep 1.0 for the standard version.
        self.router_residual_scale = float(
            _cfg(config, 'router_residual_scale', 1.0)
        )

        # Tiny noise only for the shared mask initialization.
        ...
        if self.router_hidden_dim <= 0:
            raise ValueError(
                "router_hidden_dim must be > 0."
            )

        if self.router_residual_scale < 0.0:
            raise ValueError(
                "router_residual_scale must be >= 0."
            )

        # -------------------------------------------------------------
        # Separate item embeddings for the two edge-conditioned routed graph views.
        ...
        # -------------------------------------------------------------
        # SHARED BACKBONE + EDGE-CONDITIONED COMPETITIVE PRIVATE ROUTER
        # -------------------------------------------------------------

        # -------------------------------------------------------------
        # Endpoint degree statistics
        # -------------------------------------------------------------
        ...
        # Compress the heavy-tailed degree distributions.
        ...
        # Endpoint degree values for every UNIQUE interaction edge.
        ...
        # -------------------------------------------------------------
        # Structural features used by the shared router MLP.
        #
        # x_e = [
        #     d_u,
        #     d_i,
        ...
        # ]
        # -------------------------------------------------------------
        degree_gap = torch.abs(
            edge_user_degree - edge_item_degree
        )
        ...
        self.edge_feature_dim = edge_features.size(1)
        ...
        # -------------------------------------------------------------
        # Fixed degree PRIOR logits.
        #
        # Multiplication by temperature makes the initial softmax routing
        # independent of the chosen temperature:
        #
        # softmax(
        ...
        # -------------------------------------------------------------
        # Shared edge-conditioned residual router.
        #
        # Unlike a free [E,2] parameter matrix, this MLP learns ONE routing
        # function that is reused across all interaction edges.
        #
        # The final layer is initialized to zero, therefore:
        #
        #     delta_logits = 0
        #
        # at initialization, so the model starts exactly from the degree prior.
        # -------------------------------------------------------------
        self.private_router_mlp = nn.Sequential(
            nn.Linear(
        ...
        nn.init.xavier_uniform_(
            self.private_router_mlp[0].weight
        )
        nn.init.zeros_(
            self.private_router_mlp[0].bias
        )

        nn.init.zeros_(
            self.private_router_mlp[2].weight
        )
        nn.init.zeros_(
            self.private_router_mlp[2].bias
        )
        ...
        # Because the residual MLP starts at exactly zero, this is the initial
        ...
        print(
            "Initial edge-conditioned routing | "
            "shared: {:.4f}, "
            "q1(niche): {:.4f}, "
    ...
    # -----------------------------------------------------------------
    # Edge-conditioned private routing
    # -----------------------------------------------------------------
    def get_router_delta_logits(self):
        """
        Learned correction produced by the shared edge-conditioned MLP.

        Returns:
            delta_logits: [E, 2]
        """
        return (
            self.router_residual_scale
            * self.private_router_mlp(
                self.edge_router_features
            )
        )

    def get_router_logits(self):
        """
        Degree prior + learned structural correction.

            final_logits
                = degree_prior_logits
                + delta_logits
        """
        delta_logits = self.get_router_delta_logits()

        return (
            self.degree_prior_logits
            + delta_logits
        )

    def get_private_routing(self):
        """
        Competitive two-way routing.
        ...
        Each edge satisfies:
            q1 + q2 = 1
        """
        router_logits = self.get_router_logits()
    ...
    def compute_branch_representations(self):
        """
        Returns:
            view1_rep:  shared + edge-conditioned niche-routed GCN representation
            view2_rep:  shared + edge-conditioned popularity-routed GCN representation
            view1_mask: [E]
            view2_mask: [E]
        """
    ...
    def fuse_representations(self, view1_rep, view2_rep):
        """
        Concatenate the two edge-conditioned competitive graph views.

        View 1:
            shared backbone + edge-conditioned niche-routed private structure.

        View 2:
            shared backbone + edge-conditioned popularity-routed private structure.
    ...
    def calculate_loss(self, interaction):
        ...
        The router learns a shared structural rule across edges.
        Degree provides the initial prior; the MLP learns the correction.
        """
        ...
        # Diagnostics only. Recompute them without building an extra autograd
        # graph because they do not contribute to the training objective.
        with torch.no_grad():
            shared_mask = torch.sigmoid(
                self.shared_mask_logits
            )
            ...
            prior_routing = F.softmax(
                self.degree_prior_logits
                / self.router_temperature,
                dim=1
            )

            routing_change_from_prior = (
            ...
            self.loss_components = {
                ...
                'routing_confidence': float(
                    routing_confidence.cpu()
                ),

                # Direct measure of how much the MLP has learned beyond degree.
                'router_delta_abs_mean': float(
                    delta_logits.abs().mean().cpu()
                ),

                'routing_change_from_degree_prior': float(
                    routing_change_from_prior.cpu()
                ),
...
class GCN(torch.nn.Module):
    ...
    parent model and are separate between the two edge-conditioned graph views.
    """
    def __init__(
        self,
        datasets,

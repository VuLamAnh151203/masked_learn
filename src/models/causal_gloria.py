...
Design:
  ...
  4) Separate item embeddings and separate GCNs for the causal/non-causal branches.
  5) The same non-causal GCN is reused with a randomized non-causal mask for intervention.
...
class CAUSAL_GLORIA(GeneralRecommender):
    def __init__(self, config, dataset):
        super(CAUSAL_GLORIA, self).__init__(config, dataset)
    ...
    def fuse_representations(self, causal_rep, noncausal_rep):
        """
        Preserve your original two-branch concatenation strategy.

        Users: concat causal and non-causal branch outputs.
        Items: concat causal and non-causal outputs, then run the existing
               item-item graph propagation.
        """
        user_causal = causal_rep[:self.num_user]
        user_noncausal = noncausal_rep[:self.num_user]

        item_causal = causal_rep[self.num_user:]
        item_noncausal = noncausal_rep[self.num_user:]

        user_rep = torch.cat(
            [user_causal, user_noncausal],
            dim=1
        )

        item_rep = torch.cat(
            [item_causal, item_noncausal],
            dim=1
        )

        user_rep = user_causal
        item_rep = item_causal
    ...
    # def fuse_representations(self, causal_rep):
    #     user_rep = causal_rep[:self.num_user]
    #     item_rep = causal_rep[self.num_user:]
    ...
    def calculate_loss(self, interaction):
        """
        Practical causal/non-causal LightGCN training objective.

        This is NOT claimed to be the exact CaGE Equation (7), because the
        complete equation was not present in the supplied source/passage.

        Components:
          1) main_bpr:
             recommendation quality using fused causal + non-causal reps.

          2) causal_bpr:
             makes the causal branch itself predictive.

          3) intervention_loss:
             keeps the pairwise recommendation margin stable when ONLY the
             non-causal mask is randomized.

          4) mask_entropy_loss:
             encourages masks away from ambiguous 0.5 values.

          5) mask_balance_loss:
             prevents the trivial solution causal_mask ~= 1 for every edge.
             causal_ratio controls the desired average causal mass.
        """
        pos_scores, neg_scores, aux = self.forward(
            interaction,
            return_aux=True
        ...
        # 1) Main recommendation objective.
        main_bpr = self.bpr_loss(pos_scores, neg_scores)

        # 2) Causal branch should itself contain predictive information.
        causal_bpr = self.bpr_loss(
            aux['causal_pos_scores'],
            aux['causal_neg_scores']
        )

        # 3) Prediction margin should be stable under intervention on non-causal mask.
        normal_margin = pos_scores - neg_scores
        random_margin = aux['random_pos_scores'] - aux['random_neg_scores']

        intervention_loss = F.mse_loss(
            random_margin,
            normal_margin.detach()
        )

        # 4) Encourage masks to become more decisive (near 0 or 1).
        causal_mask = aux['causal_mask']
        eps = 1e-8
        mask_entropy_loss = -(
            causal_mask * torch.log(causal_mask + eps)
            + (1.0 - causal_mask) * torch.log(1.0 - causal_mask + eps)
        ).mean()

        # 5) Avoid all-causal/all-noncausal collapse.
        mask_balance_loss = (
            causal_mask.mean() - self.causal_ratio
        ).pow(2)

        total_loss = (
            main_bpr
            + self.causal_bpr_weight * causal_bpr
    ...
    def full_sort_predict(self, interaction):
        # Use normal causal + normal non-causal graphs at inference time.

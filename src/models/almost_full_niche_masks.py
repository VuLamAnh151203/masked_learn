...
Initialization
--------------
View 1: almost-full graph prior

    M1^(0) = 0.95

for every user-item interaction edge.

View 2: niche-degree prior

For each interaction edge e=(u,i):

    d_u = normalized log user degree
    d_i = normalized log item degree

    niche_score_e = d_u * (1 - d_i)

Then:

    M2^(0) = mask_min + (mask_max - mask_min) * niche_score_e

Interpretation:
    - active user -> low-degree/tail item  => larger M2
    - low-activity user -> popular item   => smaller M2

Both masks are only INITIALIZED this way.
After initialization, M1 and M2 are both fully learnable edge-wise parameters.
...
This keeps exactly two GCN branches.
"""
...
class ALMOST_FULL_NICHE_MASKS(GeneralRecommender):
    def __init__(self, config, dataset):
        super(ALMOST_FULL_NICHE_MASKS, self).__init__(config, dataset)
        ...
        # View 1 starts nearly as the full graph.
        self.view1_init_mask = float(
            _cfg(config, 'view1_init_mask', 0.95)
        )

        # View 2 niche-degree prior is mapped into this probability range.
        ...
        # Small symmetry-breaking noise added in logit space.
        self.init_logit_noise = float(
            _cfg(config, 'init_logit_noise', 1e-2)
        )

        if not (0.0 < self.view1_init_mask < 1.0):
            raise ValueError(
                "Require 0 < view1_init_mask < 1."
            )
        ...
        # -------------------------------------------------------------
        # Separate item embeddings for the almost-full and niche-degree views.
        ...
        #
        # View 1:
        #   starts at a constant almost-full mask, e.g. 0.95.
        #
        # View 2:
        #   starts from a niche-degree prior:
        #
        ...
        #
        # IMPORTANT:
        # these are initialization priors only. Both masks are nn.Parameter
        # tensors and are free to change during BPR optimization.
        # -------------------------------------------------------------

        # -------------------------------------------------------------
        # Compute user/item degree from the training interaction graph.
        # -------------------------------------------------------------
        ...
        # Recommendation graphs usually have a heavy-tailed degree
        # distribution, so compress degree with log(1 + degree).
        ...
        # Endpoint degree corresponding to every UNIQUE user-item edge.
        ...
        # -------------------------------------------------------------
        # View 1 initialization: nearly full graph.
        # -------------------------------------------------------------
        view1_init_mask = torch.full(
            (self.num_interactions,),
            self.view1_init_mask,
            dtype=torch.float32,
            device=self.device
        )

        # -------------------------------------------------------------
        # View 2 initialization: niche-degree prior.
        #
        # High when:
        #   user degree is high
        #   item degree is low
        ...
        # Convert probabilities to logits because get_masks() applies sigmoid.
        ...
        # Buffers for diagnostics / later analysis.
        ...
        print(
            "Initial masks | "
            "view1(almost-full) mean: {:.4f}, "
            "view2(niche-degree) mean: {:.4f}".format(
    ...
    def get_masks(self):
        ...
        Shapes:
            view1_mask: [E]  # almost-full initialized
            view2_mask: [E]  # niche-degree initialized

        Initialization only provides the starting prior.
        Both masks are free during optimization.
        """
    ...
    def fuse_representations(self, view1_rep, view2_rep):
        ...
        View 1 starts almost full.
        View 2 starts from a niche-degree prior.
    ...
    def calculate_loss(self, interaction):
        ...
        The almost-full and niche-degree structures are initialization priors,
        not constraints. No preservation loss is applied.
        """
        ...
        # Diagnostics only; they do NOT affect optimization.
        with torch.no_grad():
            self.loss_components = {
                'bpr': float(
                    bpr_loss.detach().cpu()
                ...
                'view1_change_from_init': float(
                    (view1_mask - self.initial_view1_mask)
                    .abs()
                    .mean()
                    .detach()
                    .cpu()
                ...
                'view2_change_from_init': float(
                    (view2_mask - self.initial_view2_mask)
                    .abs()
                    .mean()
                    .detach()
                    .cpu()
...
class GCN(torch.nn.Module):
    ...
    parent model and are separate between the almost-full and niche-degree views.
    """
    def __init__(
        self,
        datasets,

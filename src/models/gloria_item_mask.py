# coding: utf-8
"""
GLORIA with a learnable ITEM-LEVEL two-branch graph split.

Main idea
---------
Instead of manually splitting items into low-degree / high-degree groups, every
item i learns a 2-way routing distribution:

    [m_i^(1), m_i^(2)] = softmax(item_mask_logits[i] / temperature)

with

    m_i^(1) + m_i^(2) = 1.

Every user-item interaction (u, i) uses the SAME item routing weights for item i.
Therefore all edges connected to item i are softly routed to the same two graph
branches.
...
This is deliberately NOT called causal/non-causal. The two branches are latent
specialized interaction views learned only for recommendation unless additional
causal objectives are introduced.
"""
...
class GLORIA_ITEM_MASK(GeneralRecommender):
    def __init__(self, config, dataset):
        super(GLORIA_ITEM_MASK, self).__init__(config, dataset)
        ...
        # -------------------------------------------------------------
        # Learnable item-mask hyperparameters.
        # -------------------------------------------------------------
        # Higher temperature -> softer masks.
        # Lower temperature -> more decisive branch assignments.
        self.mask_temperature = float(
            _cfg(config, 'mask_temperature', 1.0)
        )

        # Small positive entropy weight encourages confident assignments.
        self.mask_entropy_weight = float(
            _cfg(config, 'mask_entropy_weight', 1e-3)
        )
        ...
        # -------------------------------------------------------------
        # ITEM-LEVEL learnable two-way mask.
    ...
    # -----------------------------------------------------------------
    # Learnable ITEM masks
    # -----------------------------------------------------------------
    def get_item_masks(self):
        """
        Return one 2-way soft routing distribution per item.

        Shape:
            item_masks: [num_item, 2]

        Every row sums to 1:
            item_masks[i, 0] + item_masks[i, 1] = 1
        """
        temperature = max(
    ...
    def get_forward_edge_masks(self, item_masks=None):
        """
        Convert item masks [num_item, 2] to unique forward edge masks [E, 2].

        All interactions involving the same item receive the same two mask
        values because routing is item-level, not edge-level.
        """
        if item_masks is None:
            item_masks = self.get_item_masks()
    ...
    def compute_branch_representations(self):
        """
        Build both learned graph branches.
    ...
    def calculate_loss(self, interaction):
        ...
        Objective:
            ...
            L = BPR
                + lambda_entropy * item-mask entropy
                + lambda_balance * collapse prevention
                + optional embedding L2

        The router is learned end-to-end because BPR gradients flow through:

            scores -> fused reps -> GCN messages -> edge masks
                   -> item masks -> item_mask_logits
        """
    ...
    def get_item_routing_statistics(self):
        ...
        active_masks = item_masks[active]
        ...
        stats = {
            'item_masks': item_masks.detach(),
            'active_item_masks': active_masks.detach(),
            'active_item_degree': active_degree.detach(),
            'hard_branch': hard_branch.detach(),
            'branch_usage': branch_usage.detach(),
...
class Base_gcn(MessagePassing):
    ...
    The degree normalization uses the original graph topology. The learnable
    item mask changes message strength but does not recompute graph degrees.
    """
    def __init__(
        self,
        in_channels,

# coding: utf-8
"""
GLORIA variant: FULL GRAPH + INFORMATION-BOTTLENECK MASKED GRAPH.
...
Branch 2:
    stochastic learnable bottleneck graph

        pi_e = sigmoid(mask_logits[e])

    During training, an approximately discrete Concrete / Gumbel-Sigmoid
    sample is drawn for every unique interaction edge:

        m_e = sigmoid(
            (mask_logits[e] + logistic_noise_e)
            / temperature
        )
...
----------------------
The masked branch is encouraged to retain only a limited amount of graph
information while the fused recommender must still solve the ranking task.

We use a Bernoulli KL bottleneck:

    KL_e =
        pi_e * log(pi_e / rho)
        + (1-pi_e) * log((1-pi_e)/(1-rho))

where rho is a low edge-retention prior.
...
Interpretation:
    * full branch = broad / stable collaborative information
    * IB branch   = compressed, recommendation-useful structural information
...
This version keeps exactly TWO expensive GCN forward passes.
"""
...
class FULL_INFORMATION_BOTTLENECK_MASK(GeneralRecommender):
    def __init__(self, config, dataset):
        super(FULL_INFORMATION_BOTTLENECK_MASK, self).__init__(config, dataset)
        ...
        # -------------------------------------------------------------
        # Information-bottleneck mask settings.
        # -------------------------------------------------------------

        # Initial expected retention probability of each bottleneck edge.
        ...
        # Lower rho => stronger compression pressure.
        self.ib_prior_retention = float(
            _cfg(config, 'ib_prior_retention', 0.30)
        )
        ...
        # Lower => more discrete masks.
        self.ib_temperature = float(
            _cfg(config, 'ib_temperature', 1.0)
        )

        # Optional straight-through hard mask:
        #
        #   forward: 0/1
        #   backward: relaxed Concrete gradient
        #
        # Default False is usually safer for the first experiment.
        self.ib_hard_mask = bool(
            _cfg(config, 'ib_hard_mask', False)
        )

        # At evaluation, False uses expected probability pi_e.
        # True thresholds pi_e at ib_eval_threshold.
        self.ib_eval_hard_mask = bool(
            _cfg(config, 'ib_eval_hard_mask', False)
        )

        self.ib_eval_threshold = float(
            _cfg(config, 'ib_eval_threshold', 0.50)
        )
        ...
        if self.ib_temperature <= 0.0:
            raise ValueError(
                "ib_temperature must be > 0."
            )

        if not (0.0 < self.ib_eval_threshold < 1.0):
            raise ValueError(
                "Require 0 < ib_eval_threshold < 1."
            )

        # -------------------------------------------------------------
        # Separate item embeddings for:
        #   1) full/general branch
        #   2) information-bottleneck branch
        #
    ...
    # -----------------------------------------------------------------
    def get_ib_probabilities(self):
        """
        Expected edge-retention probabilities:

            pi_e = sigmoid(logit_e)
    ...
    def sample_ib_mask(self):
        """
        Concrete / Gumbel-Sigmoid relaxation for Bernoulli edge retention.
        ...
        If ib_hard_mask=True, use a straight-through 0/1 mask.
        """
        logits = self.ib_mask_logits
        ...
        # forward uses hard_mask, backward uses relaxed_mask.
        return (
            hard_mask
            - relaxed_mask.detach()
            + relaxed_mask
    ...
    def get_eval_ib_mask(self):
        """
        Deterministic mask used for evaluation.

        Default:
            expected soft mask pi_e.

        Optional:
            thresholded hard mask.
        """
        pi = self.get_ib_probabilities()

        if not self.ib_eval_hard_mask:
            return pi

        return (
            pi >= self.ib_eval_threshold
        ).to(
            pi.dtype
        )
    ...
    def ib_kl_loss(self):
        ...
        averaged across interaction edges.
        """
        pi = self.get_ib_probabilities().clamp(
            min=self.ib_eps,
            max=1.0 - self.ib_eps
        )
    ...
    # -----------------------------------------------------------------
    def compute_branch_representations(self, sample_ib=None):
        """
        Returns:
            full_rep: [num_user + num_item, D]
            ib_rep:   [num_user + num_item, D]
            ib_mask:  [E]
            ib_prob:  [E]
        ...
        ib_rep, ib_preference = self.ib_gcn(
            ...
            edge_mask=ib_edge_mask
        )

        self.full_preference = full_preference
        self.ib_preference = ib_preference
    ...
    def fuse_representations(self, full_rep, ib_rep):
        """
        Concatenate:
        ...
        user_ib = ib_rep[:self.num_user]

        item_full = full_rep[self.num_user:]
        item_ib = ib_rep[self.num_user:]
...
class GCN(torch.nn.Module):
    ...
    The full and information-bottleneck branches own separate GCN instances,
